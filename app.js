...
const TAB_KEY = "poc.tab.v1";         // list view tab: "servers" (processes) or "machines" (hosts)

const state = {
  data: null,                      // status.json (list)
  detail: null,                    // detail/<num>.json (detail view)
  machine: null,                   // machine/<agent>.json (machine view)
  tab: loadTab(),                  // list view: "servers" | "machines" (persisted)
  sortKey: "num",
...
function loadTab() { try { return localStorage.getItem(TAB_KEY) === "machines" ? "machines" : "servers"; } catch { return "servers"; } }
...
function render() {
  ...
  const machines = state.tab === "machines";
  document.getElementById("list").hidden = machines;
  document.getElementById("mlist").hidden = !machines;
  document.getElementById("shead").hidden = machines;
  document.getElementById("mhead").hidden = !machines;
  document.getElementById("sorts").hidden = machines;      // sort keys are server metrics; machines sort by use%
  if (machines) return renderMachines(f);
...
/* ---- machines tab: one row per HOST (status.json.machines, from the agents' [Agentinfo] push).
   Sorted by memory use% descending - the whole point of the tab is "which box is tight". ---- */
function memClass(p) { return p == null ? "" : p < 70 ? "ok" : p < 85 ? "warn" : "bad"; }
function renderMachines(f) {
  const all = state.data?.machines || [];
  const rows = all.filter(m => !f || (m.name || "").toLowerCase().includes(f) ||
                               (m.ip || "").includes(f) ||
                               (m.servers || []).some(n => n.toLowerCase().includes(f)));
  rows.sort((a, b) => (b.use_pct ?? -1) - (a.use_pct ?? -1) || natCmp(a.name || "", b.name || ""));
  const list = document.getElementById("mlist");
  list.innerHTML = "";
  for (const m of rows) list.appendChild(machineRowEl(m));
  document.getElementById("foot").textContent = all.length
    ? `${rows.length} / ${all.length} machines`
    : "no machine data (bridge needs the agent-info build)";
}
function machineRowEl(m) {
  const el = document.createElement("div");
  el.className = "mrow" + (m.stale ? " stale" : "");
  el.addEventListener("click", () => navToMachine(m.name));
  const cls = memClass(m.use_pct);

  const name = document.createElement("span");
  name.className = "m-name";
  name.innerHTML = `<b></b><small></small>`;
  name.querySelector("b").textContent = m.name || "?";
  name.querySelector("small").textContent = m.ip || "";

  const bar = document.createElement("span");
  bar.className = "m-bar";
  bar.innerHTML = `<span class="membar"><i class="${cls}" style="width:${Math.min(100, Math.max(0, m.use_pct || 0))}%"></i></span>` +
                  `<small></small>`;
  bar.querySelector("small").textContent =
    m.used_gb == null ? "—" : `${m.used_gb} / ${m.total_gb ?? "?"} GB`;

  el.append(name, bar,
            el_span("m-pct " + cls, m.use_pct == null ? "–" : m.use_pct + "%"),
            el_span("m-cpu", m.cpu == null ? "–" : m.cpu + "%"),
            el_span("m-srv", String((m.servers || []).length || "–")));
  return el;
}
...
function renderDetail() {
  ...
  const mb = document.getElementById("d-machine");       // host this process runs on (server key's agent field)
  if (mb) {
    mb.hidden = !d.machine;
    if (d.machine) { mb.textContent = "Host · " + d.machine; mb.dataset.machine = d.machine; }
  }
...
function setRangeButtons() {   // the From–To panel is always visible; just reflect which range is active
  document.querySelectorAll("#d-ranges .rbtn, #m-ranges .rbtn")
    .forEach(b => b.classList.toggle("active", b.dataset.r === state.range));
}
...
function renderMUpdated() { paintAge(document.getElementById("m-updated"), state.machine, "mock"); }

/* ---------- machine (host) detail: machine/<agent>.json ----------
   Same two-buffer history shape as a server detail, so the shared range window applies; the
   metrics differ because these are the BOX's numbers - machine-wide CPU across all cores (a
   single-threaded GS pegging one core reads ~100% in the server view while its host sits at 5%)
   and physical memory rather than one process's working set. */
function renderMachine() {
  const d = state.machine;
  if (!d) return;
  const c = d.current || {};
  ...
  const cls = memClass(c.use_pct);
  const cells = [
    ["Memory", c.used_gb == null ? "—" : `${c.used_gb} GB`, `of ${c.total_gb ?? "?"} GB`, cls],
    ["Used", c.use_pct == null ? "—" : c.use_pct + "%", "physical", cls],
    ["Available", c.avail_gb == null ? "—" : `${c.avail_gb} GB`, ""],
    ["Machine CPU", c.cpu == null ? "—" : c.cpu + "%", "all cores", cpuClass(c.cpu)],
    ["Network", c.net_pct == null ? "—" : c.net_pct + "%", "of NIC"],
    ["Game procs", c.proc_mem_gb == null ? "—" : `${c.proc_mem_gb} GB`,
     `${(c.servers || []).length} server${(c.servers || []).length === 1 ? "" : "s"}`],
  ];
  document.getElementById("m-current").innerHTML = cells.map(([label, val, sub, k]) =>
    `<div class="cur"><span class="cl">${esc(label)}</span><span class="cv ${k || ""}">${esc(val)}</span>` +
    (sub ? `<span class="cs">${esc(sub)}</span>` : "") + `</div>`).join("");

  const srv = c.servers || [];
  document.getElementById("m-srv-cnt").textContent = srv.length ? `${srv.length}` : "";
  document.getElementById("m-servers").innerHTML = srv.length
    ? srv.map(n => `<button class="srvchip" data-num="${esc(n)}">${esc(dispNum(n))}</button>`).join("")
    : `<div class="nochart">No game servers report from this host${c.cpu_model ? "" : ""}</div>`;

  const disks = c.drives || [];
  document.getElementById("m-disk-card").hidden = !disks.length;
  document.getElementById("m-disks").innerHTML = disks.map(dr =>
    `<div class="disk"><span class="dk">${esc(dr.d)}:</span>` +
    `<span class="membar"><i class="${memClass(dr.use_pct)}" style="width:${Math.min(100, Math.max(0, dr.use_pct || 0))}%"></i></span>` +
    `<span class="dv">${dr.use_pct == null ? "–" : dr.use_pct + "%"}</span>` +
    `<small>${esc(((dr.total_gb - dr.free_gb) || 0).toFixed(1))} / ${esc(String(dr.total_gb))} GB</small></div>`).join("");

  setRangeButtons();
  renderMachineTrends();
}
function sliceM(buf, lo, hi) {
  const t = [], used = [], cpu = [];
  const ub = buf.used || [], cb = buf.cpu || [];
  (buf.t || []).forEach((ti, i) => {
    if (ti >= lo && ti <= hi) { t.push(ti); used.push(ub[i] ?? null); cpu.push(cb[i] ?? null); }
  });
  return { t, used, cpu };
}
function coarsenM(sl, maxPts) {
  const n = sl.t.length;
  if (n <= maxPts) return sl;
  const step = Math.ceil(n / maxPts);
  const out = { t: [], used: [], cpu: [] };
  for (let i = 0; i < n; i += step) {
    const end = Math.min(i + step, n);
    out.t.push(sl.t[i]);
    for (const k of ["used", "cpu"]) {
      let sum = 0, cnt = 0;
      for (let j = i; j < end; j++) { const v = sl[k][j]; if (v != null) { sum += v; cnt++; } }
      out[k].push(cnt ? sum / cnt : null);
    }
  }
  return out;
...
function renderMachineTrends() {
  ...
  const w = currentWindow();
  const h = d.history || {};
  const buf = (w.src === "hour" ? h.hour : h.minute) || { t: [], used: [], cpu: [] };
  const sl = coarsenM(sliceM(buf, w.lo, w.hi), TREND_MAX_PTS);
  const bands = d.maint || [];
  document.getElementById("chart-mem").innerHTML =
    lineChart(sl.used, "#f5b942", sl.t, { bands, empty: "no memory history in range yet" });
  document.getElementById("chart-mcpu").innerHTML =
    lineChart(sl.cpu, "#37c98a", sl.t, { bands, fixedMin: 0, fixedMax: 100, empty: "no CPU history in range yet" });
  const rng = histRange(sl.t);
  document.getElementById("mm-range").textContent = rng;
  document.getElementById("mc-range").textContent = rng;
}
...
function currentRoute() {
  ...
  m = h.match(/^#m\/(.+)$/);
  if (m) return { view: "machine", name: decodeURIComponent(m[1]) };
  return { view: "list" };
}
...
function navToMachine(name) { if (name) location.hash = "m/" + encodeURIComponent(name); }
...
function showView() {
  ...
  document.getElementById("machine-view").hidden = r.view !== "machine";
...
function paint(r, data) {
  ...
  else if (r.view === "machine") { state.machine = data; renderMachine(); }
...
async function activeFetch(force) {
  ...
  do {
    ...
    const r = currentRoute();
    const file = r.view === "detail" ? "detail/" + encodeURIComponent(r.num) + ".json"
               : r.view === "machine" ? "machine/" + encodeURIComponent(r.name) + ".json"
               : "status.json";
    const bannerId = r.view === "detail" ? "d-banner" : r.view === "machine" ? "m-banner" : "banner";
    const have = r.view === "detail" ? state.detail : r.view === "machine" ? state.machine : state.data;
    if (!have) {                                               // cold open: paint cache instantly, no blank screen
      const c = cacheGet(file);
...
document.getElementById("tabs").addEventListener("click", e => {
  ...
  try { localStorage.setItem(TAB_KEY, state.tab); } catch {}
  for (const t of document.querySelectorAll(".tabbtn")) t.classList.toggle("active", t.dataset.tab === state.tab);
  render();
});
document.getElementById("d-machine").addEventListener("click", e => navToMachine(e.currentTarget.dataset.machine));
document.getElementById("m-servers").addEventListener("click", e => {
  const c = e.target.closest(".srvchip");
  if (c) navTo(c.dataset.num);                             // machine -> one of its servers
});
document.getElementById("m-ranges").addEventListener("click", e => {
  const b = e.target.closest(".rbtn");
  if (!b) return;
  state.range = b.dataset.r;                               // shared with the server detail selector
...
document.getElementById("refresh3").addEventListener("click", () => activeFetch(true));
...
setInterval(() => {
  ...
  if (v === "detail") renderDUpdated(); else if (v === "machine") renderMUpdated(); else renderUpdated();
}, 1000);
