...
const state = {
  ...
  filter: "",
  pinned: loadPinned(),
  range: loadRange(),              // selected trend/event window (persisted)
...
function renderAlerts() {
  ...
  bar.innerHTML =
    ...
    vis.map(a =>
      `<div class="alert alert-${esc(a.level)}">` +
...
function lineChart(vals, color, t, opts) {
  ...
  let bands = "";                                       // maintenance windows shaded behind the data
  if (opts.bands && opts.bands.length && t && t.length === n) {
    ...
    for (const b of opts.bands) {                       // [start, end|null] — null = still ongoing
      ...
      bands += `<rect class="maintband" x="${x1.toFixed(1)}" y="${PT}" width="${Math.max(1, x2 - x1).toFixed(1)}" height="${H - PT - PB}"><title>Maintenance</title></rect>`;
    }
  }
  let grid = "";                                        // vertical marks at UTC midnight (epoch % 86400 == 0)
  if (t && t.length === n) {
    for (let i = 0; i < n - 1; i++) {
      const a = t[i], b = t[i + 1];
      if (!(b > a)) continue;
      for (let m = Math.ceil(a / 86400) * 86400; m <= b; m += 86400) {   // each UTC midnight between two points
        if (m <= a) continue;
        const gx = X(i + (m - a) / (b - a)).toFixed(1);                  // interpolate onto the index axis
        grid += `<line class="daygrid" x1="${gx}" y1="${PT}" x2="${gx}" y2="${H - PB}" vector-effect="non-scaling-stroke"/>`;
      }
    }
  }
...
function sliceRange(buf, sec) {
  const lo = Date.now() / 1000 - sec, t = [], ccu = [], fps = [], cpu = [];
  const cb = buf.cpu || [];
  (buf.t || []).forEach((ti, i) => { if (ti >= lo) { t.push(ti); ccu.push(buf.ccu[i]); fps.push(buf.fps[i]); cpu.push(cb[i] == null ? null : cb[i]); } });
  return { t, ccu, fps, cpu };
}
...
function renderTrends() {
  ...
  const sl = sliceRange(pickBuffer(d.history || {}, r.src), r.sec);
  ...
  const lo = Date.now() / 1000 - r.sec;
  const ev = (d.events || []).filter(e => (e && e.ep) ? e.ep >= lo : true);   // events without ep (mock) always shown
...
document.getElementById("alertbar").addEventListener("click", e => {
  ...
  else if (one) dismissAlert(one.dataset.id, false);
});
